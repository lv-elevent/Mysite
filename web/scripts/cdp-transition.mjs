...
//
// 用法：node scripts/cdp-transition.mjs <baseUrl> <outDir> [热点下标] [总时长ms] [--mobile]
//
// --mobile 用 390×844 + 触摸模拟（不开触摸模拟会走桌面分支，见 cdp-camera-trace.mjs）。
//
...
const OUT = pos[1] ?? 'shots/transition'
const IDX = Number(pos[2] ?? 3) // 0=个人信息 1=实习与项目 2=兴趣与旅行 3=联系方式
const DURATION = Number(pos[3] ?? 2000)
...
async function main() {
  ...
  try {
    ...
    else await setViewport(cdp, { w: 1280, h: 800, dpr: 1 })
